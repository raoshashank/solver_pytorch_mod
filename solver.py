...
class Solver:
  ...
  def __init__(self, FLAGS, is_master=True):
    ...
    if self.is_master and FLAGS.SOLVER.log_wandb:
      wandb.init(
        project='occlusion_aware_detection',
        group='antares_run012',
        job_type=FLAGS.SOLVER.run,
        config={
          "last_layer": FLAGS.WANDB.last_layer,
          "Loss function": FLAGS.WANDB.loss_function,
          "regularization" : FLAGS.WANDB.reg,
          "epochs": FLAGS.SOLVER.max_epoch,
          "full depth": FLAGS.DATA.train.full_depth,
          "depth": FLAGS.DATA.train.depth,
          "conditioning" : FLAGS.WANDB.conditioning,
          "batch_size": FLAGS.DATA.train.batch_size,
          "name": FLAGS.WANDB.network,
          "notes":FLAGS.WANDB.notes,
  ...
  def config_model(self):
    ...
    if self.world_size > 1:
      ...
      model = torch.nn.parallel.DistributedDataParallel(
          ...
          find_unused_parameters=self.FLAGS.SOLVER.find_unused_parameters)
    # if self.is_master:
    #   print(model)
    self.model = model
    if self.is_master and self.FLAGS.SOLVER.log_wandb:
      wandb.watch(model)
  ...
  def train_epoch(self, epoch):
    ...
    if self.is_master:
      ...
      if self.FLAGS.SOLVER.log_wandb:
        avg = train_tracker.average()
        wandb.log(avg)
